...
{
    public partial class OpinionList : BasePage
    {
        ...
        private void BindData()
        {
            try
            {
                ...
                int recordCount = 0;
                using (DataTable dt = SqlPagerHelper.GetTableByCondition(DefaultConnection.ConnectionStringByDefaultDB, "*", "[view_opinion]", condition.ToString(), "ORDER BY ID asc", Grid1.PageIndex + 1, Grid1.PageSize, out recordCount))
                {
        ...
        protected void Grid1_RowCommand(object sender, FineUI.GridCommandEventArgs e)
        {
            if (e.CommandName == "Delete")
            {
                ...
                int flag = operate.DeleteOpinionByID(id);
                if (flag >= 1)
                {
