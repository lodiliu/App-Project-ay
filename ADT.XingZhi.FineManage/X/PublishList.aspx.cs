...
{
    public partial class PublishList : BasePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                btnAdd.OnClientClick = Window1.GetShowReference("EditPublish.aspx", "编辑");
                BindData();
            }
        }
        ...
        private void BindData()
        {
            try
            {
                ...
                int recordCount = 0;
                using (DataTable dt = SqlPagerHelper.GetTableByCondition(DefaultConnection.ConnectionStringByDefaultDB, "*", "[view_Publish]", condition.ToString(), "ORDER BY createdtime desc", Grid1.PageIndex + 1, Grid1.PageSize, out recordCount))
                {
        ...
        {
            return Window2.GetShowReference("CommentList.aspx?id=" + id, "编辑");
        }
