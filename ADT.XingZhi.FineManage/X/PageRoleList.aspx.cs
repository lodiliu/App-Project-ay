...
{
    public partial class PageRoleList : BasePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                btnAdd.OnClientClick = Window1.GetShowReference("EditPageRole.aspx", "新增");
                BindData();
            }
        }
        ...
        private void BindData()
        {
            try
            {
                ...
                int recordCount = 0;
                using (DataTable dt = SqlPagerHelper.GetTableByCondition(DefaultConnection.ConnectionStringByDefaultDB, "*", "[view_page]", condition.ToString(), "ORDER BY PageCode asc", Grid1.PageIndex + 1, Grid1.PageSize, out recordCount))
                {
        ...
        protected void Grid1_RowCommand(object sender, FineUI.GridCommandEventArgs e)
        {
            if (e.CommandName == "Delete")
            {
                ...
                int flag = operate.DeletePageRoleByID(id);
                if (flag >= 1)
                {
            ...
            else if (e.CommandName == "CheckBox1")
            {
                ...
                int a = operate.UpdatePageRoleByID(id, checkState);
                if (a > 0)
                {
