<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="ListRenqi.aspx.cs" Inherits="ADT.XingZhi.API.ListRenqi" %>
...
<head>
 ...
 -->
    ...
    <script type="text/javascript">
        ...
        function readyReg() {
            //获取li的个数
            var liLength = document.getElementById("RenqiList").getElementsByTagName('li').length;
        ...
        function LoadMore() {
            ...
            if (qs) {
                for (var i = 0; i < qs.length; i++) {
                    ...
                    else if (qs[i].substring(0, qs[i].indexOf('=')) == "orderby") {
                        orderby = qs[i].substring(qs[i].indexOf('=') + 1);
                    }
            ...
            $.ajax({
                type: "get",
                dataType: "json",
                url: "API/GetList/GetListByRenqi_Share?type=" + type + "&gender=" + gender + "&orderby=" + orderby + "&pagesize=" + pagesize + "&pageindex=" + pages + "&length=" + liLength,
                success: function (data) {
                    $("#RenqiList").append(data);
                    //获取已有的页数
                    var haspages = document.getElementById("RenqiList").getElementsByTagName('li').length
...
<body>
    ...
    <div class="container">
        <div class="ranking-list-con">
            <ul class="ranking-list popularity-list" id="RenqiList">
                <%=LoadInfo() %>
            </ul>
