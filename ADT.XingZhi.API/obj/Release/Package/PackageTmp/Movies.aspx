<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Movies.aspx.cs" Inherits="ADT.XingZhi.API.Movies" %>
...
<head runat="server">
 ...
 -->
    ...
    <script type="text/javascript">
        ...
        function PageLoad() {
            //获取li的个数
            var liLength = document.getElementById("MoviesList").getElementsByTagName('li').length;
            ...
            if (qs) {
                for (var i = 0; i < qs.length; i++) {
                    ...
                    else if (qs[i].substring(0, qs[i].indexOf('=')) == "type") {
            ...
            $.ajax({
                type: "get",
                dataType: "json",
                url: "API/GetFindPlay/GetFindPlayByAll?userid=" + userid + "&type=" + type + "&pagesize=" + pagesize + "&pageindex=" + pageindex,
                success: function (data) {
                    $("#MoviesList").append(data);
                    //获取总行数
                    var totallines = $(".pxLine").attr("data-id");
                    //获取已有的页数
                    var haslines = document.getElementById("MoviesList").getElementsByTagName('li').length
        ...
        function LoadMore() {
            ...
            $.ajax({
                type: "get",
                dataType: "json",
                url: "API/GetFindPlay/GetFindPlayByAll?userid=" + userid + "&type=" + type + "&pagesize=" + pagesize + "&pageindex=" + pages,
                success: function (data) {
                    $("#MoviesList").append(data);
                    //获取总行数
                    var totallines = $(".pxLine").attr("data-id");
...
<body>
    <!-- 影视剧 -->
    <div class="container">
        <ul class="rn-media-list" id="MoviesList">
        </ul>
